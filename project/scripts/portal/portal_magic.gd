...
enum Mode { FREE, AIMING, BOOSTING, BOOST_AIM, EDITING, MARKED, CUT_CHARGE }
...
func launch(target: Dictionary) -> void:
	...
	mode = Mode.FREE; slow_left = 0; hover_left = 0
...
func hint() -> String:
	match mode:
		Mode.AIMING: return "出口飞行 %.0f m · 松左键立即飞过去" % guide.distance
		...
		Mode.MARKED: return "切割点锁定 · 按住右键蓄力 / 左键转为移动"
		Mode.CUT_CHARGE: return "切割直径 %.1f m · 松右键展开双门" % (cut_radius * 2)
